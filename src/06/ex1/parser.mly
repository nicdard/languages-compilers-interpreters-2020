...
expr:
    ...
    | IF b = expr THEN e1 = expr ELSE e2 = expr %prec prec_let
        { If(b, e1, e2) }
    | FUN f = ID LPAR x = ID COLON t1 = ttype RPAR COLON t2 = ttype EQUAL body = expr IN scope = expr %prec prec_let
        { Letfun(f, x, Tfun(t1, t2), body, scope) }
